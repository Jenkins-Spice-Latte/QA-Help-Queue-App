...
public class UpdateControllerIntegrationTest {
	...
	void updateTest() throws Exception{
		String mockUpdate = this.jsonifier.writeValueAsString(TEST_TICKET_2);
		
		this.mockMvc.perform(put("/update/" + TEST_TICKET_2.getTicketID()).accept(MediaType.APPLICATION_JSON)
									.contentType(MediaType.APPLICATION_JSON).content(mockUpdate))
									.andExpect(status().isAccepted())
									.andExpect(content().json(mockUpdate));
