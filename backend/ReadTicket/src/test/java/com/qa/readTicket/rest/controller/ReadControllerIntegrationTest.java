...
public class ReadControllerIntegrationTest {
	...
	@Test
	void realAllTest() throws Exception {
		
		String mockList = this.jsonifier.writeValueAsString(TICKETS);
		
		this.mockMvc.perform(get("/readAll").accept(MediaType.APPLICATION_JSON))
						.andExpect(status().isOk())
						.andExpect(content().json(mockList));
	}
	
	@Test
	void readByIdTest() throws Exception{
		
		String mockTicket = this.jsonifier.writeValueAsString(TEST_TICKET_4);
		
		this.mockMvc.perform(get("/read/" + (TEST_TICKET_4.getTicketID())).accept(MediaType.APPLICATION_JSON))
									.andExpect(status().isOk())
							        .andExpect(content().json(mockTicket));
